...
echo "  - Max sequence length: 1536"
echo "  - Batch size: 2 (QLoRA 4-bit optimized)"
echo "  - Gradient accumulation: 9 (effective batch size: 18)"
echo "  - Epochs: 3"
echo "  - Learning rate: 2e-4"
echo "  - LoRA rank: 64"
echo "  - LoRA alpha: 16"
echo ""
echo "Speed & Memory Optimizations:"
echo "  - Quantization: 4-bit NF4 (QLoRA - minimal quality loss)"
echo "  - Precision: bfloat16 compute"
echo "  - torch.compile: ENABLED (reduce-overhead mode for 30-40% speedup)"
echo "  - Gradient checkpointing: DISABLED (allows torch.compile to work)"
echo "  - Eval accumulation steps: 8 (prevents OOM during validation)"
echo "  - Optimizer: adamw_8bit (memory efficient)"
echo ""
